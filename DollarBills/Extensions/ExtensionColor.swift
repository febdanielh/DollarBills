//
//  ExtensionColor.swift
//  DollarBills
//
//  Created by Febrian Daniel on 13/10/23.
//

import SwiftUI

extension Color {
    //Yellow Light
    static var YellowLight1 = Color("Yellow: Light 1")
    static var YellowLight2 = Color("Yellow: Light 2")
    static var YellowLight22 = Color("Yellow: Light 2.2")
    static var YellowLight23 = Color("Yellow: Light 2.3")
    static var YellowLight3 = Color("Yellow: Light 3")
    
    //Yellow Normal
    static var YellowNormal = Color("Yellow: Normal")
    static var YellowNormal2 = Color("Yellow: Normal 2")
    
    //Yellow Dark
    static var YellowDark1 = Color("Yellow: Dark 1")
    static var YellowDark2 = Color("Yellow: Dark 2")
    static var YellowDark23 = Color("Yellow: Dark 2.3")
    static var YellowDark25 = Color("Yellow: Dark 2.5")
    static var YellowDark26 = Color("Yellow: Dark 2.6")
    static var YellowDark3 = Color("Yellow: Dark 3")
    static var YellowDark4 = Color("Yellow: Dark 4")
    static var YellowDark5 = Color("Yellow: Dark 5")
    
    static var TextDimGray = Color("Text Dim Gray")
    static var WPBG = Color("WeeklyPerformanceBackground")
    static var PBBG = Color("ProgressBarBackground")
    static var SheetGray = Color("Sheet Gray")
    static var emptyProgressBar = Color("EmptyProgressBar")
    
    static let redStopButton = Color("Red Stop Button")
    static let blueResumeButton = Color("Blue Resume Button")
    
    static let leaderboardOrange = Color("Leaderboard Orange")
    static let leaderboardLightOrange = Color("Leaderborad LightOrange")
}
